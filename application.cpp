...
class utility
{
  ...
  static bool isvalidname(string s)
  {
    ...
    if(!isfileempty())      //proceed only if file is not empty, if file is empty then name doesnt exist in file 
    {
      ...
      {
        getline(reader,line);
        if(line=="") return true;       // no more lines in file, string s not found in file
...
class application
{
  ...
  static void read()
  {
    ...
    else
    {
      ...
      {
        getline(reader,s);
        if(s!="") cout<<s.substr(0,10)<<"\t"<<s.substr(10)<<endl;
      }
      reader.close();
...
int main()
{
  ...
  { 
    ...
    if(ch=='v' || ch=='V') application::read();
    else if(ch=='s' || ch=='S') application::search();
    else if(ch=='n' || ch=='N') application::add();
    else if(ch=='d' || ch=='D') application::deleter();
    else if(ch=='a' || ch=='A') 
    {
      system("CLS");
      cout<<"Developed by Aman Kumar\n"
      <<"\n\tAbout the developer\n"
      <<"Student at Panjab University, Chandigarh\n"
      <<"Class: MCA 2nd semester\n"
      <<"Roll No: 57\n"
      <<"\n\tAbout the application\n"
      <<"It is an standalone application developed using C++\n"
      <<"Topics covered while developing this application are:\n"
      <<"Classes, Public access modifier and static members in C++\n"
      <<"File Handling in C++\n"
      <<"String manipulation in C++\n"
      <<"Input Output streams in C++\n"
      <<"Functions and various programming constructs of C++\n\n"
      <<"Press any key to open MAIN MENU >>";
      getch();
      system("CLS");
    } 
